...
#define RAISE_IF_ERROR(rc) if ((rc) == SSH_ERROR) libssh_ruby_raise(ssh_channel_get_session(holder->channel))
...
static size_t
rb_channel_memsize(RB_UNUSED_VAR(const void *arg))
{
  return sizeof(ChannelHolder);
}

static VALUE
m_initialize(VALUE self, VALUE session)
{
...
static VALUE
m_request_exec(VALUE self, VALUE cmd)
{
  ChannelHolder *holder;

  TypedData_Get_Struct(self, ChannelHolder, &channel_type, holder);
  RAISE_IF_ERROR(ssh_channel_request_exec(holder->channel, StringValueCStr(cmd)));

  return Qnil;
}

static VALUE
m_read(int argc, VALUE *argv, VALUE self)
{
  ChannelHolder *holder;
  ...
  nbytes = ssh_channel_read_timeout(holder->channel, buf, c_count, RTEST(is_stderr) ? 1 : 0, FIX2INT(timeout));
...
void Init_libssh_channel(void)
{
  ...
  rb_define_method(rb_cLibSSHChannel, "initialize", RUBY_METHOD_FUNC(m_initialize), 1);
  rb_define_method(rb_cLibSSHChannel, "open_session", RUBY_METHOD_FUNC(m_open_session), 0);
  rb_define_method(rb_cLibSSHChannel, "close", RUBY_METHOD_FUNC(m_close), 0);
  rb_define_method(rb_cLibSSHChannel, "request_exec", RUBY_METHOD_FUNC(m_request_exec), 1);
